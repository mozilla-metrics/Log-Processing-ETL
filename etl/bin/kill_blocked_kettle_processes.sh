...
for KETTLE_PID in $KETTLE_PROCESSES ;
do
	$DEBUG found kettle pid = $KETTLE_PID
	KETTLE_LOG_FILES=`ls -l /proc/$KETTLE_PID/fd | egrep '.log$' | cut -d '>' -f 2 2>/dev/null`
	...
	if [ -z "$OLD_LOG_FILES" ]; then
		echo "========================================================================================================"
		echo "Found a Kettle process (PID=$KETTLE_PID) without logs activity for at least $STALL_TIME_THRESHOLD minutes."
